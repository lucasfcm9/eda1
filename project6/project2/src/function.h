...
long double avg(int *matrix, int initial_line, int limit_line, int initial_column, int limit_column)
{
	long double sum = 0;
...
int *binaryMatrix(int *matrix, long double avg, int initial_line, int limit_line, int initial_column, int limit_column)
{
...
long double *features_generator(int *a, int *b, int *c, int *d, int *e, int *f, int *g, int *h)
{

	int i, j;
	long double *features = (long double *)calloc(24, sizeof(long double));
...
long double *concatenation_vector(int *ILBP_vector, long double *GLCM_vector)
{
	long double *reallocation_of_memory;
	int i, j, x, k;
	long double *concatenation_ILBP_GLCM;
	concatenation_ILBP_GLCM = (long double *)malloc(536 * sizeof(long double));
...
long double *n_vector(long double *c_vector)
{
	long double min=0.0, max=1.0;
...
long double *normalized_sum(long double *vector, long double *sum_concatenated_vector)
{
	...
	for (i = 0; i < 536; i++)
	{
		*(sum_concatenated_vector + i) += *(vector + i);
		fprintf(file, "%Lf ", sum_concatenated_vector[i]);
	}
	fprintf(file, "\n");
...
long double *normalized_average(long double *vector)
{
	int i;
	long double sum = 0.0;
...
long double *final_result_training(char f, int *random_numbers)
{
	...
	long double *concatenation_ILBP_GLCM;
	long double *normalized_vector;
	long double *sum_normalized;
	long double *average_normalized;
	int initial_line = 0, limit_line = 2;
	int initial_column = 0, limit_column = 2;

	long double *sum_concatenated_vector = (long double *)calloc(536, sizeof(long double));
...
int dist_euclidiana(int *random_numbers, long double *final_grass, long double *final_asphalt, int n, char type)
{
	char txt[][50] = {"01.txt", "02.txt", "03.txt", "04.txt", "05.txt", "06.txt", "07.txt", "08.txt", "09.txt", "10.txt", "11.txt",
		"12.txt", "13.txt", "14.txt", "15.txt", "16.txt", "17.txt", "18.txt", "19.txt", "20.txt", "21.txt", "22.txt",
		"23.txt", "24.txt", "25.txt", "26.txt", "27.txt", "28.txt", "29.txt", "30.txt", "31.txt", "32.txt", "33.txt",
	...
	long double *normalized_vector;
	int *matrix;
	long double sum_dist_asphalt = 0;
	long double sum_dist_grass = 0;
	long double root_result_grass;
	long double root_result_asphalt;
	int result;
	...
	for (int i = 0; i < 536; i++)
	{
		sum_dist_grass += pow(fabsl(*(normalized_vector + i) - *(final_grass + i)), 2);
		sum_dist_asphalt += pow(fabsl(*(normalized_vector + i) - *(final_asphalt + i)), 2);
	}
