...
typedef struct _pista{
  bool ocupado;
  struct lista_voo *voo;
}Pista;
...
int main(){
  ...
  //Gerando as pistas
  int contador_pista[3]  = {0,0,0};
  Pista pista[3];
  pista[0].ocupado = false;
  pista[1].ocupado = false;
  pista[2].ocupado = false;
  ...
  while(count != 4){
    Lista_voo *atual;
    for(atual = fila.ini, i = 0; atual != NULL; atual = atual->prox){
      if(i == 2){
        if(atual->modo == 'D'){
          pista[i].voo = atual;
          pista[i].ocupado = true;
          break;
        }else{
          printf("Modo no atual do else %c\n", atual->modo);
        }
      }else if(i < 2){
        pista[i].voo = atual;
        pista[i].ocupado = true;
    ...
    for(int j  = 0; j < 3; j++){
      if(pista[j].ocupado == true && pista[j].voo->modo == 'A'){
        contador_pista[j]++;
        printf("Pista ocupada para A na pista %d\n", j);
      }
      if(contador_pista[j] == 4 && pista[j].voo->modo == 'A'){
        pista[j].ocupado = false;
        contador_pista[j]= 0;
        printf("Pista desocupada para A na pista %d\n", j);
      }
      if(pista[j].ocupado == true && pista[j].voo->modo == 'D'){
        contador_pista[j]++;
        printf("Pista ocupada para D na pista %d\n", j);
      }
      if(contador_pista[j] == 2 && pista[j].voo->modo == 'D'){
        pista[j].ocupado = false;
        contador_pista[j]= 0;
        printf("Pista desocupada para D na pista %d\n", j);
      }
    }
    count++;
  }
